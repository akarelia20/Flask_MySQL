from flask_app.config.mysqlconnection import MySQLConnection, connectToMySQL
from flask_app.models import message
from flask import flash
import re


EMAIL_REGEX = re.compile(r'^[a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$')

class User:
    db = 'private_wall'
    def __init__(self, data):
        self.id = data['id']
        self.first_name = data['first_name']
        self.last_name = data['last_name']
        self.email = data['email']
        self.password = data['password']
        self.created_at = data['created_at']
        self.updated_at = data['updated_at']
        self.messeger = None
        self.received_message = None
        self.count = 0

    @staticmethod
    def validate_registration(data):
        is_valid = True
        if len(data["first_name"]) <= 1:
            flash("First Name must be at least 2 characters.", "register")
            is_valid = False
        if len(data["last_name"]) <= 1:
            flash("Last Name must be at least 2 characters.", "register")
            is_valid = False
        if len(data["password"]) < 8:
            flash("Password must be atleast 8 characters.", "register")
            is_valid = False
        if not EMAIL_REGEX.match(data['email']):
            is_valid = False 
            flash("Invalid email address!", "register")
        if data['password'] != data['confirm_password']:
            is_valid = False 
            flash("Passwords do not match, try entering it again", "register")
        
        query = "SELECT * FROM users where email = %(email)s"
        result= connectToMySQL(User.db).query_db(query,data)
        if len(result) >= 1:
            is_valid = False
            flash('Email already exist in database!!', "register")
        return is_valid

    @staticmethod
    def validate_message_legnth(data):
        is_valid = True
        if len(data['message']) <= 5:
            flash ("Message must be at least 5 characters long!!!", "message")
            is_valid = False
        return is_valid

    @classmethod
    def save(cls,data):
        query = "INSERT INTO users (first_name, last_name, email, password) VALUES (%(first_name)s, %(last_name)s, %(email)s, %(password)s);"
        return MySQLConnection(cls.db).query_db(query,data) #method returns users.id from database

    @classmethod
    def get_all_registered_users(cls,data):
        query = "SELECT * FROM users WHERE id != %(id)s  ORDER BY first_name ASC;"
        results = MySQLConnection(cls.db).query_db(query,data)
        print(results)
        list_of_registered_users= []
        for row in results:
            list_of_registered_users.append(cls(row))
        print(list_of_registered_users)
        return list_of_registered_users


    @classmethod
    def get_user_by_id(cls,data):
        query = "SELECT * FROM users where id = %(id)s;"
        results = connectToMySQL(User.db).query_db(query,data)
        if len(results) < 1:
            return False
        print(f"#############{results[0]}")
        return cls(results[0])

    @classmethod
    def get_user_by_email(cls,data):
        query = "SELECT * FROM users where email= %(email)s;"
        results = connectToMySQL(User.db).query_db(query,data)
        if len(results) < 1:
            return False
        return cls(results[0])

    #displays the recieved messages buy a logged-user
    @classmethod
    def all_received_messages(cls,data):
        query = "SELECT * from users join messages on users.id = messages.users_id WHERE reciever_id = %(id)s;"
        results = connectToMySQL(User.db).query_db(query,data)
        user_with_recieved_message = []
        for row in results:
            logged_user = cls(row)
            data = {
                "id" : row['messages.id'],
                "users_id": row['users_id'],
                "reciever_id": row['reciever_id'],
                "message" : row['message'],
                "created_at" : row['messages.created_at'],
                "updated_at" : row['messages.updated_at']
            }
            msg = message.Message(data)
            logged_user.received_message = msg
            data2 = {
                "id" : row['users_id']
            }
            # messager contains message and person who send it
            logged_user.messeger = cls.get_user_by_id(data2)
            user_with_recieved_message.append(logged_user)
        print(user_with_recieved_message)
        return user_with_recieved_message
    
    def messages_sent(self,count):
        self.count += 1
        print(f"***************************************{self.count}")
        return self.count
        
    def count_messages(self):
        return f" You have sent {self.count} messeges so far"

    